...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] =
    if (initial.isEmpty) Stream.empty
      ...
      val more = for {
        (b, history) <- initial
        newNeighbor <- newNeighborsOnly(neighborsWithHistory(b, history), explored)
      } yield newNeighbor
  ...
  lazy val solution: List[Move] = {
    def solution_(initial: Stream[(Block, List[Move])]): List[Move] = initial match {
      case Stream() => List()
      case (b, moves) #:: rest => if (done(b)) moves.reverse else solution_(rest)
    }
    solution_(pathsFromStart)
  }
